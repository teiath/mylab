...
function GetWorkerSpecializations($pagesize, $page) {
    global $db;
    global $Options;
    global $app;
    ...
    try {
        ...
        $sort = array( new DSC(WorkerSpecializationsExt::FIELD_NAME, DSC::ASC) );

        $oWorkerSpecializations = new WorkerSpecializationsExt($db);
        $totalRows = $oWorkerSpecializations->findByFilterAsCount($db, $filter, true);
        $result["total"] = $totalRows[0]->getWorkerSpecializationId();
        
        if ($pagesize)
            $countRows = $oWorkerSpecializations->findByFilterWithLimit($db, $filter, true, $sort, $startat, $pagesize);
        else
            $countRows = $oWorkerSpecializations->findByFilter($db, $filter, true, $sort);
            
        $result["count"] = count( $countRows );

        foreach ($countRows as $row) {
            $result["data"][] = array("worker_specializations_id" => $row->getWorkerSpecializationId(), 
                                      "name" => $row->getName()
                                );
        }
