...
function GetRelationTypes($pagesize, $page) {
    global $db;
    global $Options;
    global $app;
...
//==============================================================================  
        ...
        $sort = array( new DSC(RelationTypesExt::FIELD_NAME, DSC::ASC) );

        $oRelationType = new RelationTypesExt($db);
        $totalRows = $oRelationType->findByFilterAsCount($db, $filter, true);
        $result["total"] = $totalRows[0]->getRelationTypeId();
        
        if ($pagesize)
            $countRows = $oRelationType->findByFilterWithLimit($db, $filter, true, $sort, $startat, $pagesize);
        else
            $countRows = $oRelationType->findByFilter($db, $filter, true, $sort);
        
        $result["count"] = count( $countRows );

        foreach ($countRows as $row){
            $result["data"][] = array("relation_type_id" => $row->getRelationTypeId(), 
                                      "name" => $row->getName()
                                );
        }
