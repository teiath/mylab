...
function GetLabSources($pagesize, $page) {
    global $db;
    global $app;
    ...
    try {
        ...
        //sorting lab_sources by name and Initialize object $oLabSources
        $sort = array( new DSC(LabSourcesExt::FIELD_NAME, DSC::ASC) );
        $oLabSources = new LabSourcesExt($db);
        
        //find total results by filter
        $totalRows = $oLabSources->findByFilterAsCount($db, $filter, true);
        $total = $totalRows[0]->getLabSourceId();
        ...
        if ($pagesize)        
            $oLabSources->getAllWithLimit($db, $filter, true, $sort, $startAt, $pagesize);
        else
            $oLabSources->getAll($db, $filter, true, $sort );
        
        //find total results by filter with limits of $page and $pagesize
        $result["count"] = count( $oLabSources->getObjsArray() );

        //loop for results
        foreach ($oLabSources->getObjsArray() as $oSource)
        {
            $result["data"][] = array( "source_id" => (int)$oSource->getLabSourceId() , 
